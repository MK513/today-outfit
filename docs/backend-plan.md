...
| 3. 옷장 · 이미지 | `/clothes` 조회(필터) · 등록 · 일괄 등록 · 상세 · 삭제, `/images` 업로드(형식 · 크기 검사) | 사진을 base64 대신 URL로 저장 | ✅ 완료 |
...
| 사진 저장소 | `ImageStore` 인터페이스 + `LocalImageStore`(로컬 디스크 `uploads/images/{userId}/{UUID}.{jpg\|png}`), `app.image.storage`로 구현 선택 | 별도 인프라 없이 시작하고, S3 등으로 바꿀 때 구현체 하나만 추가. 검증 · 소유권 · 정리 규칙은 `ImageService`에 있어 그대로 유지. 새 구현은 `ImageStoreContractTest`를 통과해야 함 |
| 사진 DB 저장값 | `clothes.image_url` 컬럼에 URL 대신 저장소 키(`{userId}/{UUID}.png`) 저장(V3), 응답의 `image_url`은 서버가 키로 만듦 | 저장소나 공개 URL이 바뀌어도 DB를 고칠 필요가 없음. 컬럼명은 명세(DBML)대로 유지 |
| 사진 조회 권한 | `GET /api/images/**`는 토큰 없이 허용 | `<img>` 태그는 인증 헤더를 보낼 수 없음. 파일명이 추측 불가한 UUID라 URL을 아는 사람만 볼 수 있음 |
| 사진 검증 | Content-Type · 확장자가 아니라 파일 앞부분 시그니처로 JPG/PNG 판별, 5MB 초과 · 빈 파일 거부 | 이미지로 위장한 HTML 등 업로드 차단 |
| 사진 연결 | 의류 저장 시 `image_url`은 본인이 업로드한 파일만 허용 | 남의 사진이나 외부 URL 연결 방지 |
| 사진 정리 | 의류 삭제 시 파일 삭제, 탈퇴 시 사용자 폴더 삭제, 저장 안 된 업로드는 24시간 후 삭제(1시간마다 검사) | 명세 "미저장 사진은 24시간 후 정리" 현재는 저장소 전체를 나열해 확인하며, S3로 옮길 때 업로드 기록 테이블이나 S3 lifecycle 규칙으로 바꾼다 |
| 옷장 화면 데이터 | 프론트는 로그인 시 옷장 전체를 불러와 메모리에 두고 필터링 | 코디 · 플래너 · 챌린지 화면이 의류를 ID로 바로 찾아야 함. 서버 필터 API(`GET /clothes?category=...`)는 구현 · 테스트 완료 |
...
- 데모 체험 시 샘플 의류는 서버 것을 쓰고, 샘플 코디 · 배치는 아직 프론트(localStorage)에서 서버 의류로 만든다. 4단계에서 `src/lib/seedDemo.js`를 제거한다.
- 4~5단계 전까지 코디 · 플래너는 여전히 localStorage에 있고, 서버 의류 ID를 참조한다. 서버 도입 전에 만든 로컬 의류 데이터는 앱 시작 시 삭제된다.
- 사진 분석(`POST /ai/clothes/photo-analysis`)은 6단계 전까지 목업이라, 사진 등록 화면은 저장할 때 `POST /images`로 사진을 올린다.
